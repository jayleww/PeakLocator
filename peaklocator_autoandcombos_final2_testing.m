function varargout = peaklocator_autoandcombos_final2_testing(varargin)
% PEAKLOCATOR_AUTOANDCOMBOS_FINAL2_TESTING MATLAB code for peaklocator_autoandcombos_final2_testing.fig
%      PEAKLOCATOR_AUTOANDCOMBOS_FINAL2_TESTING, by itself, creates a new PEAKLOCATOR_AUTOANDCOMBOS_FINAL2_TESTING or raises the existing
%      singleton*.
%
%      H = PEAKLOCATOR_AUTOANDCOMBOS_FINAL2_TESTING returns the handle to a new PEAKLOCATOR_AUTOANDCOMBOS_FINAL2_TESTING or the handle to
%      the existing singleton*.
%
%      PEAKLOCATOR_AUTOANDCOMBOS_FINAL2_TESTING('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in PEAKLOCATOR_AUTOANDCOMBOS_FINAL2_TESTING.M with the given input arguments.
%
%      PEAKLOCATOR_AUTOANDCOMBOS_FINAL2_TESTING('Property','Value',...) creates a new PEAKLOCATOR_AUTOANDCOMBOS_FINAL2_TESTING or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before peaklocator_autoandcombos_final2_testing_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to peaklocator_autoandcombos_final2_testing_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help peaklocator_autoandcombos_final2_testing

% Last Modified by GUIDE v2.5 25-Nov-2016 21:00:32

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @peaklocator_autoandcombos_final2_testing_OpeningFcn, ...
                   'gui_OutputFcn',  @peaklocator_autoandcombos_final2_testing_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before peaklocator_autoandcombos_final2_testing is made visible.
function peaklocator_autoandcombos_final2_testing_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to peaklocator_autoandcombos_final2_testing (see VARARGIN)

% Choose default command line output for peaklocator_autoandcombos_final2_testing
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes peaklocator_autoandcombos_final2_testing wait for user response (see UIRESUME)
...
function varargout = peaklocator_autoandcombos_final2_testing_OutputFcn(hObject, eventdata, handles) 
...
ygausslist = [];
xgausslist = [];
cutoffpoint = ymax*0.3;
j = maxloc;
prevmin = y(j);
gaussfitcount = gaussfitcount + 1;
while (y(j) > cutoffpoint && y(j) < prevmin)
...
peaklabelled = length(peaklocationlist) + 1;
...
peaklocation = x(maxloc) %Just find where the peak actually is and label it there? Gaussian fits unnecesary? -> true only for smoothed data
...
disp(tabledata)
columnname = {'Magnetic Fields', 'A/q', 'q', 'dq'};
set(handles.uitable7, 'Data', tabledata, 'ColumnName', columnname, 'ColumnWidth', {95}, 'Rowstriping', 'on', 'ColumnEditable', [true false false false], 'CellEditCallback', @removefield);

function removefield(~, callbackdata)
global tabledata peaklocationlist
fieldrow = callbackdata.Indices(1);
peaklocationlist(fieldrow) =[];
tabledata(fieldrow, :) = [];
disp(tabledata)
...
global x;
global y;
global magfields peaklocationlist;
...
x = rawdata(:,1);
y = rawdata(:,2);
xsmoothed = smooth(x, 0.009, 'loess');
ysmoothed = smooth(y, 0.009, 'loess');
y = ysmoothed;
x = xsmoothed;
